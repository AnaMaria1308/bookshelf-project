...
function BookDetail() {
    ...
    return (
        <div className={styles.bookDetail}>
            ...
            {book.start_date && (
                <p><strong>Started:</strong> {formatDate(book.start_date)}</p>
            )}
            {book.end_date && (
                <p><strong>Finished:</strong> {formatDate(book.end_date)}</p>
            )}
            <div className={styles.buttonGroup}>
                <button onClick={handleDelete} className={styles.deleteButton}>
                    🗑️ Delete Book
                </button>
                <button onClick={() => navigate(`/editBook/${book.id}`)} className={styles.editButton}>
                    ✏️ Edit Book
                </button>
            </div>
        </div>
